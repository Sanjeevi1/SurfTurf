        ...
        <div className="fixed bottom-4 right-4 max-w-sm w-full bg-white shadow-lg rounded-lg p-4 border border-gray-300">
            <div className="h-96 overflow-y-auto space-y-4 mb-4 p-2">
                {messages.map((msg, idx) => {
                    ...
                    const textWithoutImages = msg.text.replace(/(https?:\/\/[^\s]+?(?:\.jpg|\.jpeg|\.png|\.gif))/g, '');

                    return (
                        <div key={idx} className={`message ${msg.sender === 'chatbot' ? 'bg-blue-100 text-blue-900' : 'bg-gray-100 text-gray-900'} p-3 rounded-lg max-w-xs ${msg.sender === 'user' ? 'ml-auto' : 'mr-auto'} transition-all ease-in-out duration-200`}>
                            <strong className="font-semibold">{msg.sender === 'chatbot' ? 'Bot' : 'You'}:</strong>
                            <span
                            ...
                            {imageUrls && (
                                <div className="grid grid-cols-2 gap-2 mt-2">
                                    {imageUrls.map((url, index) => (
                                        <img key={index} src={url} alt="Image" className="w-full h-auto rounded-lg" />
                                    ))}
                                </div>
                            )}
                        </div>
