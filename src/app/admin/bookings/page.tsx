    ...
    useEffect(() => {
        async function fetchUserData(userID) {
            try {
                ...
                if (response.ok) {
                    setUser(data.data);  // Save user data to state
        ...
        fetchUserData(booking.user);  // Fetch user data when component mounts
    }, [booking.user]);  // Dependency array to refetch if booking.user changes
        ...
        >
            ...
            <div className="col-span-12 md:col-span-8 flex flex-col justify-between">
                <div>
                    ...
                    {user && (
                        <div className="mt-4">
                            <h5 className="text-lg font-semibold text-zinc-900 dark:text-white mb-2">User Details</h5>
                            <p className="text-sm text-zinc-900 dark:text-white">Username: {user.username}</p>
                            <p className="text-sm text-zinc-900 dark:text-white">Email: {user.email}</p>
                            <p className="text-sm text-zinc-900 dark:text-white">Phone: {user.phone}</p>
                        </div>
                    )}
                </div>
                <div className="mt-4">
    ...
    useEffect(() => {
        const fetchBookings = async () => {
            try {
                ...
                const response = await fetch('/api/bookings/ownerBooking', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
